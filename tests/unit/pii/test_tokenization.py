...
def test_tokenize_entities(service):
    entities = {"email": ["a@b.com", "c@d.com"], "phone": ["+1234567890"]}
...
def test_tokenize_tree(service):
    data = {
        ...
        "nested": {"phone": "+1234567890", "safe": "value"},
        "list": [{"ip_address": "1.2.3.4"}, "not-pii"],
    }
...
def test_tokenize_text_content_mixed(service):
    ...
    text = "Name: John Doe, SSN: 123-45-6789, " "Email: john@example.com, " "IP: 10.0.0.1, " "Card: 4111 1111 1111 1111"
    result = service.tokenize_text_content(text, enable_llm=False)
