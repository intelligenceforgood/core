...
def run_query(*, question: str, backend: str = "faiss") -> None:
    ...
    try:
        store = build_vector_store(backend=backend)
    ...
    console.print(f"[cyan]🤖 Analyzing question:[/cyan] {question}\n")
...
def run_vertex_search(
    *,
    ...
    resolved_location = location or SETTINGS.vector.vertex_ai_location or "global"
    ...
    if boost_json:
        try:
            boost_payload = json.loads(boost_json)
    ...
    *,
    ...
    for rank, result in enumerate(results, start=1):
        ...
        summary = (
            ...
            or struct.get("title")
            or (struct.get("content", "")[:120] + "…" if struct.get("content") else None)
            or "<no summary>"
        )
...
def evaluate_vertex(
    *,
    ...
    scenarios = load_scenarios(Path(config) if config else None)
    client = discoveryengine.SearchServiceClient()
    ...
    for scenario in scenarios:
        ...
        console.print(f"{indicator} {'PASS' if passed else 'FAIL'} — {scenario.name}")
...
def refresh_hybrid_schema_snapshot(
    *,
    api_base: str,
    api_key: str | None = None,
    timeout: int = 10,
    output: str | Path = "data/hybrid_schema.json",
    indent: int = 2,
) -> None:
    """Fetch `/reviews/search/schema` and write a local snapshot."""

    url = f"{str(api_base).rstrip('/')}/reviews/search/schema"
    request = Request(url)
    ...
    console.print(f"[green]✅ Wrote schema snapshot to {out}[/green]")
