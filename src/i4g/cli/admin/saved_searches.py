...
def export_saved_searches(
    *,
    ...
    owner_filter = None if all else (owner or None)
    records = store.list_saved_searches(owner=owner_filter, limit=limit)
    include_tags_set = {t.strip().lower() for t in (include_tags or []) if t.strip()}
    if include_tags_set:
        records = [r for r in records if include_tags_set.intersection({t.lower() for t in (r.get("tags") or [])})]
    effective_schema_version = schema_version.strip() if schema_version else ""
    for record in records:
        record.pop("created_at", None)
        if record.get("tags") is None:
            record["tags"] = []
        if effective_schema_version:
            params = record.get("params") or {}
            if isinstance(params, dict):
                params["schema_version"] = effective_schema_version
    ...
    if split and output:
        ...
        for record in records:
            rec_owner = record.get("owner") or "shared"
            by_owner.setdefault(rec_owner, []).append(record)
        for rec_owner, rows in by_owner.items():
            fname = base / f"saved_searches_{rec_owner}.json"
        ...
        if output:
            ...
            console.print(f"[green]\u2705 Exported {len(records)} saved search(es) to {output}")
        else:
            console.print(data)
...
def import_saved_searches(
    *,
    ...
    store = build_review_store()
    content = Path(input).read_text() if input else sys.stdin.read()
    try:
    ...
    for item in items:
        try:
            ...
            if include_tags_set and not include_tags_set.intersection({t.lower() for t in (req.tags or [])}):
                skipped += 1
                continue
            store.import_saved_search(req.model_dump(), owner=None if shared else owner)
            imported += 1
...
def prune_saved_searches(
    *,
    ...
    store = build_review_store()
    records = store.list_saved_searches(owner=owner, limit=1000)
    tags_filter = {t.strip().lower() for t in (tags or []) if t.strip()}
    to_delete = []
    for record in records:
        record_tags = {t.lower() for t in (record.get("tags") or [])}
        if tags_filter and not tags_filter.intersection(record_tags):
            continue
    ...
    for record in to_delete:
        rec_owner = record.get("owner") or "shared"
        console.print(f"[cyan]- {record.get('name')} (owner={rec_owner}, tags={record.get('tags')})")
...
def bulk_update_saved_search_tags(
    *,
    ...
    if not any([add, remove, replace is not None]):
    ...
    normalized_remove = [t.strip() for t in (remove or []) if t.strip()]
    normalized_replace = [t.strip() for t in (replace or []) if t.strip()] if replace is not None else None

    summary_records = []
    target_ids = []

    if search_id:
        target_ids = [sid for sid in search_id if sid.strip()]
        for sid in target_ids:
            record = store.get_saved_search(sid)
            if record:
                summary_records.append(record)
    else:
        records = store.list_saved_searches(owner=owner, limit=limit)
        tags_filter = {t.strip().lower() for t in (tags or []) if t.strip()}
        if tags_filter:
    ...
    if dry_run:
        ...
        for record in summary_records[:10]:
            rec_owner = record.get("owner") or "shared"
            console.print(f"  - {record.get('name')} (owner={rec_owner}, tags={record.get('tags') or []})")
...
def export_tag_presets(*, owner: str | None = None, output: str | None = None) -> None:
    """Export tag presets derived from saved searches."""

    store = build_review_store()
    presets = store.list_tag_presets(owner=owner, limit=1000)
    data = json.dumps(presets, indent=2)
    if output:
        Path(output).write_text(data)
        console.print(f"[green]\u2705 Exported {len(presets)} tag preset(s) to {output}")
    else:
        console.print(data)


def import_tag_presets(*, input: str | Path | None = None) -> None:
    ...
    if not presets:
        ...
        return
    out = json.dumps(presets, indent=2)
    if input:
        Path(input).write_text(out)
