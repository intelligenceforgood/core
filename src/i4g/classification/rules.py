...
from i4g.taxonomy.enums import DeliveryChannel, RequestedAction
...
def detect_signals(text: str) -> dict[str, list[ScoredLabel]]:
    ...
    """
    signals: dict[str, list[ScoredLabel]] = {"actions": [], "channel": []}
    ...
    if URL_RE.search(text):
        signals["actions"].append(
            ScoredLabel(
                label=RequestedAction.CLICK_LINK.value, confidence=1.0, explanation="Detected URL/Link pattern."
            )
        )
    ...
    if PHONE_RE.search(text):
        signals["channel"].append(
            ScoredLabel(label=DeliveryChannel.PHONE.value, confidence=1.0, explanation="Detected phone number pattern.")
        )
    ...
    if EMAIL_RE.search(text):
        signals["channel"].append(
            ScoredLabel(
                label=DeliveryChannel.EMAIL.value, confidence=1.0, explanation="Detected email address pattern."
            )
        )
