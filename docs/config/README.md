...
| ssi_job | `ssi_job.core_api_url` | `I4G_SSI_JOB__CORE_API_URL`<br />`SSI_JOB_CORE_API_URL`<br />`SSI_JOB__CORE_API_URL` | `str` | `https://api.intelligenceforgood.org` | Core API base URL for task status callbacks from the SSI job. |
| ssi_job | `ssi_job.job_name` | `I4G_SSI_JOB__JOB_NAME`<br />`SSI_JOB_NAME`<br />`SSI_JOB__JOB_NAME` | `str` | `ssi-investigate` | Cloud Run Job name for the SSI investigate job. |
| ssi_job | `ssi_job.playbook_dir` | `I4G_SSI_JOB__PLAYBOOK_DIR`<br />`SSI_PLAYBOOK_DIR`<br />`SSI_JOB__PLAYBOOK_DIR` | `str` | `config/playbooks` | Directory containing SSI playbook JSON files. Resolved relative to project root. |
| ssi_job | `ssi_job.project` | `I4G_SSI_JOB__PROJECT`<br />`SSI_JOB_PROJECT`<br />`SSI_JOB__PROJECT` | `str` | `i4g-dev` | GCP project ID where the SSI Cloud Run Job is deployed. |
| ssi_job | `ssi_job.region` | `I4G_SSI_JOB__REGION`<br />`SSI_JOB_REGION`<br />`SSI_JOB__REGION` | `str` | `us-central1` | GCP region for the Cloud Run Job. |
| ssi_job | `ssi_job.service_account` | `I4G_SSI_JOB__SERVICE_ACCOUNT`<br />`SSI_JOB_SERVICE_ACCOUNT`<br />`SSI_JOB__SERVICE_ACCOUNT` | `str &#124; NoneType` | `None` | Service account to impersonate when triggering the job. |
...
| storage | `storage.retention_days` | `I4G_STORAGE__RETENTION_DAYS`<br />`STORAGE__RETENTION_DAYS`<br />`STORAGE_RETENTION_DAYS` | `int` | `90` | Number of days after resolution before automated purge. Set via I4G_STORAGE__RETENTION_DAYS. |
| storage | `storage.retention_enabled` | `I4G_STORAGE__RETENTION_ENABLED`<br />`STORAGE__RETENTION_ENABLED`<br />`STORAGE_RETENTION_ENABLED` | `bool` | `True` | Master switch for automated retention purge. Set I4G_STORAGE__RETENTION_ENABLED=false to disable. |
| storage | `storage.retention_grace_days` | `I4G_STORAGE__RETENTION_GRACE_DAYS`<br />`STORAGE__RETENTION_GRACE_DAYS`<br />`STORAGE_RETENTION_GRACE_DAYS` | `int` | `30` | Days after soft-delete before hard purge. Set via I4G_STORAGE__RETENTION_GRACE_DAYS. |
