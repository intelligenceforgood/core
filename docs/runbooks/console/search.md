...
- Core API (core-svc) reachable through IAP (`docs/design/iam.md` lists the enrollment steps).
- `X-API-KEY` with analyst scope exported as `I4G_API_KEY`.
- Convenience shell variables:
  ```bash
  export CORE_API_BASE=${CORE_API_BASE:-https://core-svc-y5jge5w2cq-uc.a.run.app}
...
1. Fetch the latest taxonomy, datasets, and presets:
   ...
   curl -sS \
     ...
     "$CORE_API_BASE/reviews/search/schema" | jq . > /tmp/hybrid_schema.json
   ```
...
2. Execute the search endpoint:
   ...
   curl -sS \
     ...
     -X POST "$CORE_API_BASE/reviews/search/query" \
...
I4G_API_URL=$CORE_API_BASE I4G_API_KEY=$I4G_API_KEY pnpm --filter web dev
```
...
5. Verify via API and UI:
   ```bash
   curl -sS -H "X-API-KEY: $I4G_API_KEY" "$CORE_API_BASE/reviews/search/saved" | jq '.items[] | {name, tags, params}'
   ```
