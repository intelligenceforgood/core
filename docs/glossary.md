...
- **Ingestion pipeline** — End-to-end flow from classified payload to structured store, SQL dual-write, and optional vector/Vertex/Firestore fan-out. See [tdd.md](tdd.md#6-ingestion-flow-canonical) and [architecture.md](architecture.md).
...
- **Hybrid search** — Combines vector retrieval with structured filters from SQL entities; powered by `HybridRetriever`. See [tdd.md](tdd.md#7-apis-and-contracts-current-surface) and [architecture.md](architecture.md).
