...
Before generating artifacts, set a run date and reuse `$RUN_DATE` everywhere:

```bash
export RUN_DATE=$(date +%Y%m%d)
```

The bootstrap flow expects bundles staged in `gs://i4g-dev-data-bundles/` (or passed via `--bundle-uri`). Use timestamped subfolders keyed by `$RUN_DATE`.

## Golden bundle (recommended)

The golden bundle consolidates all sources into a single quality-filtered package. Build it with the ETL scripts, then run the consolidation.

### 1. Clean legacy Azure data
...
Filters: min 50-char text, SHA-256 dedup, drops malformed records. Documents what was dropped in stdout.

### 2. ETL incident report responses

Export the Google Sheet as CSV first (requires manual auth), then:
...
Extracts entities from indicator columns (wallet, email, phone, URL, bank). Rejects rows with < 50 chars narrative.
...
Generates SQL INSERTs for: campaigns (7), campaign-case links (40), infrastructure edges (15), watchlist items + alerts, review queue entries, intake records across 15 countries. All use `ON CONFLICT DO NOTHING` for safety.
...
1. `legacy_azure_clean/cases.jsonl` — cleaned legacy cases
2. `public_scams/` — all `.jsonl` files (kept as-is)
3. `incident_responses/cases.jsonl` — Google Sheet ETL output
4. `synthetic_coverage/full/cases.jsonl` — minus low-quality (< 50 chars)
5. `golden_seed/seed.sql` — copied to output

Output: `data/bundles/golden/{cases.jsonl, seed.sql, manifest.json}`

Options:

- `--skip-ids FILE` — text file with case IDs to exclude (one per line)
...
To add a new data source to the golden bundle:
...
4. Rebuild the golden bundle

All ETL scripts follow the same pattern: read input → normalize to `{case_id, text, entities, metadata}` schema → filter quality → write JSONL.

## Legacy bundle structure (pre-golden)

These instructions remain for reference. Set `I4G_BOOTSTRAP__USE_GOLDEN_BUNDLE` to empty or unset to use legacy bundles.
