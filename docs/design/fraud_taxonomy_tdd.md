...
```json
{
  ...
  "techniques": [{ "label": "SE.TRUST_BUILDING", "confidence": 0.76 }],
...
- **Risk Weights:** Each taxonomy definition (Intent, Technique, Action) is assigned a `risk_weight` (1-10) in `definitions.yaml`.
  - _High (8-10):_ Extortion, Crypto Transfer, Urgency.
  - _Medium (4-7):_ Romance, Imposter, Grooming.
  - _Low (1-3):_ Generic spam.
...
- **Calculation Formula:**
  ...
  _Note: The 2.5 multiplier scales the raw weighted sum (typically ~20-40 for a strong signal) to the 0-100 range._
