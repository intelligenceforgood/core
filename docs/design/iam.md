...
| **Phase 1.5**| ✅ Complete (Jul 2026) | DB-backed programmatic API Key Authentication (`api_keys` table with SHA-256 key hashing). Self-service API key management in Next.js console (`/settings/api-keys`). Admin API key provisioning & revocation. Dedicated partner ingress (`partner-api` Cloud Run with `PARTNER_MODE=true` & Cloud Armor WAF). OpenAPI Swagger UI `APIKeyHeader` and `HTTPBearer` authorization. Scope-based middleware (`require_scope`). |
| **Phase 2** | Planned (Q3 2026)      | Introduce role-specific endpoints or Cloud Run services (victim intake, LEO portal). Add device-posture checks via BeyondCorp / Context-Aware Access. Expand audit trail to cover case-level access.                                                                                                                                                                                                                                                                                                                                   |
| **Phase 3** | Planned (Q4 2026)      | Evaluate non-Google identity options (passkeys, Auth0 for Nonprofits). Automate IAM drift detection. Implement signed report attestations for legal workflows.                                                                                                                                                                                                                                                                                                                                                                         |
